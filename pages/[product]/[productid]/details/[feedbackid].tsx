...
async function AddComment() {
  ...
  return (
    <div className=" bg-project-medium-gray p-4 h-fit relative">
      ...
      <Link href={`/products/${props.query.productid}`}>
        ...
        <div>
        <Button className="bg-regal-blue" onClick ={globalState.createEditfeedbacktoggle} >
            <span className="text-xs font-bold">Edit Feedback</span>
        </Button>
        </div>
      
      </div>
      <FeedBackComponent feedbackTitle={commentData.title} feedbackComment={commentData.content} feedbackTag={commentData.tag} feedbackUpvotes="0" commentAmount={replyData.length}/>
      
      <div className="bg-white sm:container mx-auto w-100 rounded-xl p-8 dark:bg-gray-0 flex flex-column mt-4">
      <div className="font-bold">
        {replyData.length} Comments
      </div>
      {showReplys()}
        </div>   

          <div className={`bg-white mt-5 w-100 justify-center items-center bg-white sm:container mx-auto rounded-xl p-8 flex flex-column mt-4`}>
        <Form onSubmit={handleSubmit} className="w-100">
          <Form.Group  controlId="body">
            <Form.Label className="text-lg font-bold mb-4">Add Comment <span className="text-xs font-light">(Max 250 characters)</span></Form.Label>
            <Form.Control
              type="body"
              value={commentBody}
    ...
    <Button type="submit" disabled={!validateForm()} className="mt-4 p-2 bg-fuchsia-600 border-none font-bold">
            Post Comment
          </Button>
    </div>
